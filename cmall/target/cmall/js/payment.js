...
$(document).ready(function () {
    var pay_res = setInterval(function () {
        var orderNo = $("#orderNo").val();
        $.ajax({
            method: "POST",
            url: "./query_order_pay_status.do?orderNo="+orderNo,
            success: function (res) {
                if (res.status == '0'&& res.data == true) {
                    clearInterval(pay_res);
                    $.MsgBox.Alert("消息", "支付成功！确认后跳转到购物界面...",function(){
                        clearInterval(pay_res);
...
$.MsgBox = {
    Alert: function (title, msg,callback) {
        GenerateHtml("alert", title, msg,callback);
